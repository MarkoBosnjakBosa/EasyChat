module.exports = function(app, models, async, fs, path) {
    ...
    app.delete("/deleteUser/:username", (request, response) => {
        ...
        var chatroomQuery = {$or: [{participants: username}, {blockedParticipants: username}]};
        var chatroomUpdate = {$pull: {participants: username, blockedParticipants: username}};
        queries.push(function(callback) {
            Chatroom.updateMany(chatroomQuery, chatroomUpdate).then(user => {
                callback(null, user);
            });
        });
